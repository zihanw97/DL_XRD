"""Model 3/3: CNN-SNGP-adapt (SNGP + domain adaptation).

Corresponds to the CNN-SNGP-adapt row in the paper's Table 3/4 (Sec.
4.1-4.2): CNN-SNGP with the MMD-based domain-alignment loss (global +
classwise) turned on, aligning simulated and experimental latent features
during training. This is the full model from the paper.
...
Usage:
    ...
    SKIP_TRAINING=1 python run_cnn_sngp_adapt.py
    SKIP_TRAINING=1 PRETRAINED_CHECKPOINT=/path/to/model.pth python run_cnn_sngp_adapt.py
"""
...
STUDENT_PATH = "cnn_sngp_adapt_student.pth"
...
# file, e.g.:
#   PRETRAINED_CHECKPOINT=/path/to/model.pth python run_cnn_sngp_adapt.py
PRETRAINED_CHECKPOINT = os.environ.get(
    "PRETRAINED_CHECKPOINT",
    "/mnt/a/tgf4743/migrate/HTMAX/saved_models/sngp_cnn_model_domain_adapt_all.pth",
)
...
def main():
    ...
    if SKIP_TRAINING:
        ...
        model = train_domain_adapt_semi_supervised(
            ...
            # Main losses -- domain-alignment loss ON (this is what separates
            # this model from run_cnn_sngp.py)
            source_weight=1.0,
            lambda_target_supervised=1.0,
            lambda_mmd=2.0,
            lambda_classwise_mmd=2.0,
            lambda_pseudo=0.1,
    ...
    opentsne_plot_all_latents_by_class(
        results,
        save_path="opentsne_latents_cnn_sngp_adapt.png",
