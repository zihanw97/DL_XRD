"""Model 1/3: CNN (baseline).

Corresponds to the CNN row in the paper's Table 3/4 (Sec. 4.1-4.2): the same
spectrally-normalized CNN backbone used by the other two models, but with a
plain linear+softmax output instead of the SNGP head, and no domain-alignment
(MMD) loss. This isolates what the SNGP head and domain adaptation each
contribute, evaluated the same way as run_cnn_sngp.py and run_cnn_sngp_adapt.py.
...
Usage:
    ...
    SKIP_TRAINING=1 PRETRAINED_CHECKPOINT=/path/to/model.pth python run_cnn_baseline.py
"""
...
from models import CNNWithPlainHead
from training import train_domain_adapt_semi_supervised

STUDENT_PATH = "cnn_baseline_student.pth"
...
# already produced during a prior run (guaranteed architecture match --
# unlike the legacy HTMAX checkpoints, whose "purely_cnn.pth" uses a
# different, non-spectrally-normalized head and does not load into
# CNNWithPlainHead). Override via env var, e.g.:
#   PRETRAINED_CHECKPOINT=/path/to/model.pth python run_cnn_baseline.py
PRETRAINED_CHECKPOINT = os.environ.get(
    "PRETRAINED_CHECKPOINT",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "saved_models", "cnn_baseline_student.pth"),
)
SKIP_TRAINING = os.environ.get("SKIP_TRAINING", "0") == "1"


def load_model(input_channels, output_dim, model_path=STUDENT_PATH):
    model = CNNWithPlainHead(input_channels=input_channels, output_dim=output_dim)
...
def main():
    ...
    if SKIP_TRAINING:
        ...
        model = load_model(INPUT_CHANNELS, OUTPUT_DIM, model_path=PRETRAINED_CHECKPOINT)
        model = model.to(device)
    else:
        # ---- Define & train the model ----
        model = CNNWithPlainHead(input_channels=INPUT_CHANNELS, output_dim=OUTPUT_DIM).to(device)
        ...
        model = train_domain_adapt_semi_supervised(
            ...
            # Main losses -- no domain-alignment loss for the plain CNN baseline
            source_weight=1.0,
        ...
        model = load_model(INPUT_CHANNELS, OUTPUT_DIM)
        model = model.to(device)
    ...
    # The plain CNN baseline has no SNGP head, so it has no predictive
    # variance to marginalize over -- the SNGP predictive-uncertainty
    # section from run_cnn_sngp.py / run_cnn_sngp_adapt.py is not
    # meaningful here (it would just be a zero-variance MC sample around
    # softmax(logits), i.e. exactly the plain confidence score below).
    # Skipping straight to confidence-based evaluation, the metric the
    # paper actually reports for the CNN row of Table 3/4.

    print("\n" + "=" * 70)
    print("Confidence analysis")
    print("=" * 70)
    from evaluation import compute_confidence_from_model_outputs
    ...
    opentsne_plot_all_latents_by_class(
        results,
        save_path="opentsne_latents_cnn_baseline.png",
